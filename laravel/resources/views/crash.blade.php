...
<body class="dark-bg-main">
    ...
    <style>
        ...
        .loader-plane-img {
            ...
            margin-bottom: 6px;
            animation: floatPlane 2s ease-in-out infinite alternate;
            filter: drop-shadow(0 4px 15px rgba(255, 30, 70, 0.45));
        }
    ...
    <div class="main-container">
                ...
                <div class="stage-board">
                    ...
                    <div class="loading-game">
                        <div class="center-loading text-white text-center game-centeral-loading">
                            <img src="images/flyboy10x_icon.png?v={{ env('APP_VERSION', time()) }}" class="loader-plane-img" alt="FlyBoy Plane" />
                        ...
                        <div class="bottom-left-plane">
                            <img class="plane-static" src="images/flyboy10x_icon.png?v={{ env('APP_VERSION', time()) }}" />
                        </div>
                        ...
                        <div class="controls">
                            ...
                            <div class="first-row auto-game-feature">
                                <div class="bet-block">
                                    <div class="spinner">
                                        <div class="input">
                                            <input id="bet_amount" value="5.00" type="text"
                                    ...
                                    <div class="bets-opt-list">
                                        <button class="btn btn-secondary btn-sm bet-opt main_amount_btn"
                                            onclick="select_direct_bet_amount(this);"><span
                                                class="amt">5</span><span class="currency">₦</span></button>
                                        <button class="btn btn-secondary btn-sm bet-opt main_amount_btn"
                                            onclick="select_direct_bet_amount(this);"><span
                        ...
                        <div class="controls">
                            ...
                            <div class="first-row auto-game-feature">
                                <div class="bet-block">
                                    <div class="spinner">
                                        <div class="input">
                                            <input id="bet_amount" value="5.00" class="extra_bet_amount"
                                                oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');" />
                                        </div>
                                        <div class="qty-buttons">
        ...
        <!--====== Right Sidebar End ======-->
        <!--====== Desktop Chat Sidebar Start ======-->
        <div class="ax-chat-sidebar d-none d-lg-block">
            @include('include.chatroom')
        </div>
        <!--====== Desktop Chat Sidebar End ======-->
    </div>
    ...
    <!--====== Hinal (End) ======-->
    @php
        $currentUser = session('userlogin', []);
    @endphp
    <script>
        var hash_id = '{{ csrf_token() }}';
        var currency_id = 'NGN';
        var currency_symbol = '₦';
        var wallet_balance = @json(isset($currentUser['id']) ? wallet($currentUser['id']) : 0);
        var profile_image = '1';
        var member_id = @json($currentUser['id'] ?? '');
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.isSuccess) {
                    ...
                    const notificationText = (result.data.notification || '').trim();
                    if (notificationText !== '') {
                        $("#notification_list").html('');
                        $("#notification_list").append($('<div class="notification-item"></div>').text(notificationText));
                        $("#notification_count").text('1').removeClass('d-none');
                        swal(
                            'Notification',
                            notificationText,
                            'success'
                        ...
                        $("#notification_list").html('<div class="notification-empty">No new notifications</div>');
                        $("#notification_count").addClass('d-none').text('0');
                    }
                }
            }
        });
    ...
    <div class="modal fade ax-modal" id="leaderboard-modal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content" style="background:#13141a; border:1px solid rgba(255,255,255,0.1); border-radius:14px; color:#fff;">
                <div class="modal-header d-flex align-items-center justify-content-between border-bottom border-secondary py-2 px-3">
                    <h5 class="modal-title d-flex align-items-center gap-2 m-0 fs-6 fw-bold text-white">
                        <span class="material-symbols-outlined text-warning align-middle">emoji_events</span>
                        Leaderboard & Bets
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body p-2" id="mobile-leaderboard-body">
                </div>
            </div>
        </div>
    </div>
    ...
    function setupMobileLeaderboard() {
        if (window.innerWidth < 992) {
            if ($('#mobile-leaderboard-body').length && $('.left-sidebar').length && $('.left-sidebar').parent()[0] !== $('#mobile-leaderboard-body')[0]) {
                $('.left-sidebar').appendTo('#mobile-leaderboard-body').show();
            }
        } else {
            if ($('.main-container').length && $('.left-sidebar').length && $('.left-sidebar').parent()[0] !== $('.main-container')[0]) {
                $('.left-sidebar').prependTo('.main-container').show();
            }
        }
    }
    $(document).ready(function() {
        setupMobileLeaderboard();
        $(window).on('resize', setupMobileLeaderboard);
    });
    </script>
