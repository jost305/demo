...
function scrollFunction() {
    // custom scrollbar removed; native browser scrolling is used instead.
}
// scrollFunction();
...
var min_bet_amount = 5.00;
var max_bet_amount = 50.00;

function bet_amount_incremental(element) {
    var inputEl = $(element).parent().parent().find(".input #bet_amount");
    var bet_amount = parseFloat(inputEl.val()) || 0;
    bet_amount += 5;
    if (bet_amount > max_bet_amount) bet_amount = max_bet_amount;
    if (bet_amount < min_bet_amount) bet_amount = min_bet_amount;
    inputEl.val(bet_amount.toFixed(2));
}
...
$(window).focus(function () {
    ...
    if (focus_timer > 10) {
        // Avoid forcing a full page reload on tab focus; refresh game state only.
...
document.addEventListener('visibilitychange', function (event) {
    if (document.hidden) {
        ...
        if (visibility_timer > 10) {
            // Avoid forcing a full page reload when returning to the tab; refresh game state only.
            if (typeof gamegenerate === 'function') {
                gamegenerate();
            }
        } else {
